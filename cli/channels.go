...
var cmdChannels = []cobra.Command{
	...
	},
	{
		...
		Long: "Delete channel by id.\n" +
			"Usage:\n" +
			"\tcli channels delete <channel_id> $USERTOKEN - delete the given channel ID\n",
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) != 2 {
				logUsage(cmd.Use)
				return
	...
	},
	{
		...
		Long: "List users of a channel\n" +
			"Usage:\n" +
			"\tcli channels users <channel_id> $USERTOKEN\n",
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) != 2 {
				logUsage(cmd.Use)
				return
	...
	},
	{
		...
		Long: "List groups of a channel\n" +
			"Usage:\n" +
			"\tcli channels groups <channel_id> $USERTOKEN\n",
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) != 2 {
				logUsage(cmd.Use)
				return
...
var channelAssignCmds = []cobra.Command{
	{
		...
		Long: "Assign users to a channel\n" +
			"Usage:\n" +
			"\tcli channels assign users <relation> '[\"<user_id_1>\", \"<user_id_2>\"]' <channel_id> $USERTOKEN\n",
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) != 4 {
				logUsage(cmd.Use)
				return
	...
	},
	{
		...
		Long: "Assign groups to a channel\n" +
			"Usage:\n" +
			"\tcli channels assign groups  '[\"<group_id_1>\", \"<group_id_2>\"]' <channel_id> $USERTOKEN\n",
...
var channelUnassignCmds = []cobra.Command{
	{
		...
		Long: "Unassign groups from a channel\n" +
			"Usage:\n" +
			"\tcli channels unassign groups '[\"<group_id_1>\", \"<group_id_2>\"]'  <channel_id> $USERTOKEN\n",
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) != 3 {
				logUsage(cmd.Use)
				return
...

	{
		...
		Long: "Unassign users from a channel\n" +
			"Usage:\n" +
			"\tcli channels unassign users <relation> '[\"<user_id_1>\", \"<user_id_2>\"]'  <channel_id> $USERTOKEN\n",
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) != 4 {
				logUsage(cmd.Use)
				return
